...
export default function PendingPage() {
  ...
  // --- 4. 物理初始化 (Macrotask 推遲法) ---
  useEffect(() => {
    let mounted = true;
    
    // 利用 setTimeout 將執行推遲至下一個事件迴圈 (Macrotask)
    // 徹底避開 ESLint 對「在 effect 中同步呼叫 setState」的嚴格審查
  ...
  return (
    <div className="bg-[#f7f9fb] min-h-screen font-[family-name:-apple-system,BlinkMacSystemFont,'SF_Pro_TC','PingFang_TC',system-ui,sans-serif] text-[10.5px] text-[#191c1e] antialiased tracking-tight">
      <style dangerouslySetInnerHTML={{ __html: `
        ...
        .blob-2 { bottom: -150px; left: -150px; width: 800px; height: 800px; }
      `}} />

      <div className="liquid-bg">
        <div className="blob bg-primary-fixed-dim blob-1"></div>
        <div className="blob bg-secondary-fixed blob-2"></div>
      </div>
        ...
        <header className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 mb-10">
          ...
          <div className="flex flex-col sm:flex-row items-center gap-4 w-full md:w-auto">
             ...
             <div className="relative w-full sm:w-64">
                ...
                <input title="搜尋" aria-label="搜尋" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder="搜尋單位或序號..." className="w-full pl-10 pr-4 py-2.5 bg-white/60 border border-white/60 rounded-full shadow-sm font-bold" />
             </div>
          </div>
        </header>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6 pb-24">
          {filteredData.map((r) => (
            <div 
              ...
              className={`glass-panel p-6 rounded-[1.8rem] case-card cursor-pointer border-2 ${selectedSns.has(String(r.sn)) ? 'border-primary/60 bg-blue-50/30' : 'border-white/50'}`}
            >
              <div className="flex justify-between items-start mb-4">
                <div className="flex gap-3">
                   <div className={`w-5 h-5 rounded-md border-2 flex items-center justify-center transition-all ${selectedSns.has(String(r.sn)) ? 'bg-primary border-primary' : 'bg-white border-slate-200'}`}>
                      {selectedSns.has(String(r.sn)) && <span className="material-symbols-outlined text-white text-[14px]">check</span>}
                   </div>
                   <div>
                      <h3 className="text-lg font-black text-slate-800 leading-tight truncate max-w-[150px]">{String(r.unit)}</h3>
                      <p className="text-[9px] font-bold text-slate-400 uppercase mt-0.5">{String(r.area)}棟 {String(r.floor)} | {String(r.ext)}</p>
                   </div>
                </div>
                <span className={`px-3 py-1 rounded-full font-black text-[9px] uppercase border ${r.status === '待核定' ? 'bg-orange-50 text-orange-600 border-orange-100' : 'bg-red-50 text-red-600 border-red-100'}`}>{String(r.status)}</span>
              </div>

              <div className="space-y-3 py-4 border-y border-white/40 mb-4">
                <div className="flex justify-between">
                  <span className="text-slate-400 font-bold">廠商</span>
                  <span className="font-black text-slate-700 truncate max-w-[120px]">{String(r.vendor)}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-slate-400 font-bold">序號 (S/N)</span>
                  <span className="font-mono font-black text-error">{String(r.sn)}</span>
                </div>
                <div className="flex gap-2">
                   <div className="flex-1 bg-slate-50/50 p-2 rounded-xl border text-center">
                      ...
                      <p className="font-mono font-black text-[9.5px] truncate">{String(r.mac1)}</p>
                   </div>
                   <div className="flex-1 bg-slate-50/50 p-2 rounded-xl border text-center">
                      <p className="text-[8px] font-black text-emerald-500 uppercase mb-0.5">WLAN MAC</p>
                      <p className="font-mono font-black text-[9.5px] truncate">{r.mac2 ? String(r.mac2) : 'N/A'}</p>
                   </div>
                </div>
              </div>
          ...
          <div className="glass-panel w-full max-w-lg rounded-[2.5rem] p-10 space-y-6 shadow-2xl bg-white/95 border-t-[8px] border-t-primary">
            ...
            <div className="space-y-4">
               <div className="p-4 bg-blue-50/50 rounded-2xl border">
                  ...
                  <p className="text-sm font-black text-slate-800">{String(currentCase.unit)} - {String(currentCase.model)}</p>
                  <p className="text-[10px] font-mono font-bold text-slate-400 mt-1">MAC: {String(currentCase.mac1)}</p>
               </div>
               <div className="grid grid-cols-2 gap-4">
                  <div className="col-span-2 flex flex-col gap-1.5">
                    <label className="text-[10px] font-black text-slate-500 uppercase ml-1">指定核定 IP (N)</label>
                    <input title="核定 IP" aria-label="核定 IP" value={modalForm.ip} onChange={e => setModalForm({...modalForm, ip: e.target.value})} className="font-mono text-base font-black text-blue-800" placeholder="10.x.x.x" />
                  </div>
                  <div className="flex flex-col gap-1.5">
                    <label className="text-[10px] font-black text-slate-500 uppercase ml-1">設備類型 (H)</label>
                    <select title="設備類型" aria-label="設備類型" value={modalForm.type} onChange={e => setModalForm({...modalForm, type: e.target.value})}>
                  ...
                  <div className="flex flex-col gap-1.5">
                    ...
                    <input title="設備名稱" aria-label="設備名稱" value={modalForm.name} readOnly className="bg-slate-100 font-mono text-[11px] font-black text-emerald-700" />
                  </div>
               </div>
            </div>
        ...
        <div className="fixed inset-0 z-[200] bg-slate-900/40 backdrop-blur-sm flex items-center justify-center p-6 fade-enter">
           <div className="glass-panel w-full max-w-sm rounded-[2.5rem] p-10 text-center space-y-6 bg-white/95 border-t-[8px] border-t-error">
              ...
              <div className="text-left">
                 ...
                 <textarea title="退回原因" aria-label="退回原因" value={modalForm.reason} onChange={e => setModalForm({...modalForm, reason: e.target.value})} rows={3} className="w-full mt-2 text-xs font-bold" placeholder="請詳細說明退回要求..." />
              </div>
              <div className="flex gap-3">
