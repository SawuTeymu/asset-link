 ...
 * 狀態：V300.6 CSS Modules (物理對正旗艦版)
 * 修復證明：
 * 1. 樣式脫離：移除所有 style={{...}}，將動畫與填充邏輯移至 admin.module.css。
 * 2. 結構修復：校正 div 閉合關係，杜絕 TS17002 結構報警。
 * 3. 邏輯保全：VANS 解析、MAC 對沖、Supabase 同步邏輯 0 簡化。
 * ==========================================
 */
...
export default function AdminDashboard() {
  ...
  return (
    <div className={`min-h-screen font-body-md text-on-surface antialiased flex relative overflow-hidden ${styles.medicalGradient}`}>
      ...
      <aside className={`w-64 fixed left-0 top-0 h-screen border-r border-white/20 ${styles.clinicalGlass} flex flex-col p-6 z-50 transform transition-transform duration-300 ${isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full'} md:translate-x-0`}>
          ...
          <nav className="flex-1 space-y-1">
              ...
              <button onClick={() => router.push("/pending")} className="w-full text-left p-3 text-slate-600 hover:bg-white/50 rounded-xl transition-all">行政核定池</button>
          ...
          <button onClick={() => router.push("/")} className="mt-auto flex items-center gap-2 p-3 text-red-600 font-bold hover:bg-red-50 rounded-xl">登出中樞</button>
      </aside>
        ...
        <div className="p-4 md:p-8 max-w-[1440px] mx-auto w-full flex-1">
          {activeTab === 'dashboard' && (
            <div className="animate-in fade-in duration-500">
              <div className="mb-8 px-2">
                ...
                <p className="text-sm text-slate-500 font-medium mt-1">監控全院資產節點與 IP 分配健康度。</p>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
                ...
                {/* 🚀 修復第 236 行：終極物理對沖，完全移除 style 屬性，改用 ID 選擇器與內嵌樣式 */}
              ...
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
                <div className={`${styles.clinicalGlass} col-span-1 lg:col-span-8 rounded-[2rem] p-8 h-[420px] flex flex-col shadow-xl`}>
                ...
                <div className={`${styles.clinicalGlass} col-span-1 lg:col-span-4 rounded-[2rem] p-6 flex flex-col h-[420px] shadow-xl overflow-hidden`}>
                    <h3 className="font-bold text-slate-800 mb-4 px-2">安全日誌矩陣</h3>
                    <div className="flex-1 overflow-y-auto">
                        <table className={`w-full ${styles.zebraGlass} text-xs`}>
                           <tbody>
                              {historyRecords.slice(0, 10).map((log, i) => (
                                <tr key={i}><td className="p-3 font-bold text-slate-700">{log.品牌型號}</td><td className="p-3 font-mono text-slate-400">{log.主要mac}</td></tr>
                              ))}
                           </tbody>
                        </table>
                    </div>
          ...
          {activeTab === 'vans' && (
            <div className="space-y-6 animate-in slide-in-from-bottom-4 duration-500">
              <div className={`${styles.clinicalGlass} p-10 border-l-4 border-l-blue-600 rounded-3xl shadow-xl flex flex-col md:flex-row justify-between items-center gap-6`}>
                ...
                <div className="flex gap-4">
                  ...
                  <button onClick={() => vansInputRef.current?.click()} className="px-6 py-3 bg-white border border-slate-200 text-blue-600 rounded-2xl font-black text-xs hover:bg-slate-50 shadow-sm uppercase tracking-widest">注入 CSV</button>
                  {vansData.length > 0 && <button onClick={handleSaveAudit} disabled={isSaving} className="px-8 py-3 bg-blue-600 text-white rounded-2xl font-black text-xs shadow-lg uppercase tracking-widest active:scale-95 transition-all">物理存入庫</button>}
                </div>
              </div>
              {vansData.length > 0 && (
                <div className={`${styles.clinicalGlass} rounded-3xl overflow-hidden shadow-2xl`}>
                  <table className={`w-full text-left font-mono text-xs ${styles.zebraGlass}`}>
                    <thead className="bg-slate-50"><tr><th className="p-5 font-black uppercase tracking-widest">Type</th><th className="p-5 font-black uppercase tracking-widest">IP Endpoint</th><th className="p-5 font-black uppercase tracking-widest text-right">Observation</th></tr></thead>
                    <tbody>
                      {vansConflicts.map((c, i) => (<tr key={i}><td className="p-5"><span className={`px-3 py-1 rounded-lg font-black ${c.type==='MAC_ERROR'?'bg-red-100 text-red-600':'bg-amber-100 text-amber-600'}`}>{c.type}</span></td><td className="p-5 text-blue-600 font-black">{c.ip}</td><td className="p-5 italic text-slate-400 text-right">{c.desc}</td></tr>))}
                    </tbody>
                  </table>
                </div>
              )}
      ...
      <div className="fixed bottom-6 right-6 z-[7000] flex flex-col gap-3">
        {toasts.map(t => (
          <div key={t.id} className={`px-6 py-4 rounded-2xl shadow-2xl font-bold text-xs animate-in slide-in-from-right-4 flex items-center gap-3 border ${t.type === "success" ? "bg-emerald-50 text-emerald-700 border-emerald-200" : "bg-red-50 text-red-700 border-red-200"}`}>
            <span className={`material-symbols-outlined text-base ${styles.iconFill}`}>{t.type === 'success' ? 'check_circle' : 'error'}</span><span className="tracking-widest uppercase">{t.msg}</span>
          </div>
        ))}
      </div>
    </div>
