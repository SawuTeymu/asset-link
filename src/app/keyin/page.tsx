...
function KeyinContent() {
  ...
  const loadForFix = (record: ProgressRecord) => {
    setConfirmDialog({
      isOpen: true,
      title: "載入修正",
      content: `確認將案件 [${record.sn}] 載入進行修正？提交後將物理覆蓋原紀錄。`,
      type: 'info',
      onConfirm: () => {
        setUnit(record.unit);
        setRows([{
  ...
  const handleConfirmClose = (record: ProgressRecord) => {
    setConfirmDialog({
      isOpen: true,
      title: "確認結案",
      content: `確認核定 IP [${record.assignedIp}] 正確無誤？確認後將正式遷移至歷史庫。`,
      type: 'info',
      onConfirm: async () => {
        setIsLoading(true);
        setLoaderText("執行結案數據遷移中...");
        try {
          await vendorConfirmAsset(record.sn);
          showToast("結案成功，資料已入庫歸檔");
  ...
  const handleSubmit = async () => {
    if (!vendorName || !unit || !applicant) {
      showToast("基本填報資訊未填寫完整", "error");
      return;
    }

    const hasEmpty = rows.some(r => !r.model || !r.sn || !r.mac1);
    if (hasEmpty) {
      showToast("設備明細資訊不完整", "error");
      return;
    }

    setIsLoading(true);
    setLoaderText("提交資產預約申請中...");
  ...
  return (
    <div className="min-h-screen bg-slate-50 pb-24 text-slate-900 font-sans">
      ...
      <nav className="bg-white border-b sticky top-0 z-50 px-4 py-4 flex justify-between items-center shadow-sm">
        ...
        <button onClick={fetchProgress} aria-label="查看歷史與進度" className="bg-slate-100 p-2 rounded-full hover:bg-slate-200 transition-colors">
        ...
        <section className="bg-white rounded-3xl p-6 shadow-xl border border-slate-100 space-y-4">
            ...
            <div className="grid grid-cols-1 gap-4">
                <div className="space-y-1">
                    ...
                    <input id="vendor-input" value={vendorName} onChange={e => setVendorName(e.target.value)} placeholder="維護廠商名稱" className="w-full bg-slate-50 border-none rounded-2xl px-5 py-4 focus:ring-2 focus:ring-blue-600 font-bold transition-all" />
                </div>
                <div className="grid grid-cols-2 gap-3">
                    <div className="space-y-1">
                        <label htmlFor="area-select" className="text-[10px] font-black text-slate-400 uppercase ml-1">裝機院區</label>
                        <select id="area-select" title="選擇裝機院區" value={area} onChange={e => setArea(e.target.value)} className="w-full bg-slate-50 border-none rounded-2xl px-5 py-4 font-bold">
                            <option>本院</option>
                            <option>兒醫</option>
                        </select>
                    </div>
                    <div className="space-y-1">
                        ...
                        <input id="floor-input" value={floor} onChange={e => setFloor(formatFloor(e.target.value))} placeholder="例如: 5F" className="w-full bg-slate-50 border-none rounded-2xl px-5 py-4 focus:ring-2 focus:ring-blue-600 font-bold" />
                    </div>
                </div>
                <div className="space-y-1">
                    <label htmlFor="unit-input" className="text-[10px] font-black text-slate-400 uppercase ml-1">使用單位全銜</label>
                    <input id="unit-input" value={unit} onChange={e => setUnit(e.target.value)} placeholder="例如: 資訊組" className="w-full bg-slate-50 border-none rounded-2xl px-5 py-4 focus:ring-2 focus:ring-blue-600 font-bold" />
                </div>
                <div className="grid grid-cols-2 gap-3">
                    <div className="space-y-1">
                        <label htmlFor="applicant-input" className="text-[10px] font-black text-slate-400 uppercase ml-1">申請人/分機</label>
                        <input id="applicant-input" value={applicant} onChange={e => setApplicant(e.target.value)} placeholder="王大明/1234" className="w-full bg-slate-50 border-none rounded-2xl px-5 py-4 focus:ring-2 focus:ring-blue-600 font-bold" />
                    </div>
                    <div className="space-y-1">
        ...
        {rows.map((row, index) => (
            <section key={row.id} className="bg-white rounded-3xl p-6 shadow-xl border border-slate-100 space-y-5 relative overflow-hidden">
                <div className="flex justify-between items-center">
                    ...
                    {rows.length > 1 && (
                        <button onClick={() => removeRow(row.id)} aria-label="刪除此項資產明細" className="text-red-500 material-symbols-outlined text-lg">delete</button>
                    )}
                </div>
                ...
                <div className="space-y-4">
                    ...
                    <div className="space-y-1">
                        ...
                        {row.originalSn && <p className="text-[10px] text-red-500 font-black mt-1">⚠️ 修正中: 將物理覆蓋原紀錄 {row.originalSn}</p>}
                    </div>
                    <div className="grid grid-cols-2 gap-3">
                        <div className="space-y-1">
                            <label htmlFor={`mac1-${row.id}`} className="text-[10px] font-black text-slate-400 uppercase ml-1">主要網卡 MAC</label>
                            <input id={`mac1-${row.id}`} value={row.mac1} onChange={e => updateRow(row.id, 'mac1', formatMAC(e.target.value))} placeholder="有線網路" className="w-full bg-slate-50 border-none rounded-2xl px-5 py-4 focus:ring-2 focus:ring-blue-600 font-bold" />
                        </div>
                        <div className="space-y-1">
      ...
      <footer className="fixed bottom-0 left-0 right-0 p-4 bg-white/80 backdrop-blur-xl border-t z-40 shadow-2xl flex gap-3">
          <button onClick={handleSubmit} className="flex-1 bg-blue-600 text-white rounded-2xl font-black py-4 shadow-lg active:scale-95 transition-transform flex items-center justify-center gap-2">
                  ...
                  <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-slate-50">
                      {progressData.length === 0 ? (
                          <div className="h-64 flex flex-col items-center justify-center text-slate-400">
                              ...
                              <p className="font-black text-xs uppercase tracking-widest">尚無任何歷史記錄</p>
                          </div>
                      ) : (
                          progressData.map((record, i) => (
                              <div key={i} className="bg-white rounded-2xl p-5 shadow-sm border border-slate-100 space-y-3 relative overflow-hidden group">
                                  ...
                                  <div className="flex justify-between items-start">
                                      ...
                                      <span className="text-[10px] font-bold text-slate-300 tracking-tighter">{record.date}</span>
                                  </div>
                                  
                                  <div>
                                      <p className="text-xs font-black text-slate-400 uppercase tracking-tighter">案件單號: {record.formId}</p>
                                  ...
                                  {record.status === '已核定(待確認)' && (
                                      <div className="bg-blue-50 rounded-xl p-3 border border-blue-100 space-y-2">
                                          ...
                                          <button onClick={() => handleConfirmClose(record)} className="w-full bg-blue-600 text-white py-2.5 rounded-lg font-black text-[10px] hover:bg-blue-700 transition-colors shadow-sm">確認正確無誤並結案</button>
                                      </div>
                                  )}

                                  {record.status === '退回修正' && (
                                      <div className="bg-red-50 rounded-xl p-3 border border-red-100">
                                          <p className="text-[10px] text-red-600 font-black leading-tight mb-2 flex gap-1"><span className="material-symbols-outlined text-[12px]">error</span>原因: {record.rejectReason}</p>
                                          <button onClick={() => loadForFix(record)} className="w-full bg-red-600 text-white py-2.5 rounded-lg font-black text-[10px] hover:bg-red-700 transition-colors shadow-sm">載入修正重送</button>
                                      </div>
                                  )}
                                  ...
                                  {record.status === '已結案' && (
                                      <div className="bg-emerald-50 rounded-xl p-3 border border-emerald-100 space-y-1">
                                          <div className="flex justify-between text-[10px]">
                                              <span className="font-black text-emerald-600 uppercase tracking-tighter">配發 IP</span>
                                              <span className="font-bold text-emerald-800">{record.assignedIp}</span>
                                          </div>
                                          <p className="text-[9px] text-emerald-500 font-black italic tracking-tighter text-right mt-1">此案件已物理歸檔至歷史庫</p>
                                      </div>
                                  )}
                              </div>
                          ))
...
// 主頁面入口 (封裝 Suspense 以支援 useSearchParams)
