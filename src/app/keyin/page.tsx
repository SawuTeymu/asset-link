...
export default function KeyinPage() {
  ...
  const fetchPendingRecords = useCallback(async (vName: string) => {
    ...
    setIsLoading(true);
    try {
      const { data, error } = await supabase.from("資產").select("*").eq("來源廠商", vName).order("建立時間", { ascending: false });
      if (error) throw error;
      setPendingRecords(data || []);
    } catch (err) {
      showToast("進度數據讀取失敗，請重新整理頁面", "error");
    } finally {
      setIsLoading(false);
    }
  }, [showToast]);
  ...
  const handleSubmit = async () => {
    if (isLoading) return;
    if (!metadata.area || !metadata.unit || !metadata.applicantName) { showToast("請填寫完整行政資料 (包含姓名)", "error"); return; }
    ...
    try {
      ...
      if (error) {
        if (error.code === "23505") showToast("提交失敗：序號已存在於系統中", "error");
  ...
  return (
    <div className={`min-h-screen text-slate-800 font-body-md overflow-x-hidden relative ${styles.medicalGradient} antialiased`}>
      ...
      <nav className="sticky top-0 w-full flex justify-between items-center px-4 md:px-6 h-16 bg-white/70 backdrop-blur-xl border-b border-white/40 shadow-sm z-40">
        ...
        <div className="flex items-center gap-3"><button onClick={() => router.push("/")} className="material-symbols-outlined text-slate-500 hover:text-sky-600 transition-colors">logout</button></div>
      </nav>
      ...
      <div className="flex">
        <aside className={`w-64 fixed left-0 top-0 h-screen pt-16 border-r border-white/30 bg-white/80 backdrop-blur-2xl p-6 z-50 transform transition-transform duration-300 ${isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full'} md:translate-x-0`}>
          <div className="flex justify-between items-center mb-8 px-2 font-black text-sky-800 uppercase tracking-tighter"><p className="text-lg">ALink 作業區</p></div>
          ...
          {activeTab === 'progress' && (
             <div className={`${styles.clinicalGlass} rounded-3xl p-6 md:p-8 shadow-sm flex flex-col min-h-[500px] animate-in slide-in-from-right-4`}>
                <div className="flex items-center justify-between mb-8 border-b border-slate-100 pb-4">
                  <div className="flex items-center gap-2"><span className="material-symbols-outlined text-amber-500">hourglass_empty</span><h2 className="text-lg font-bold text-slate-800 tracking-tight">審核中案件清單 ({pendingRecords.length})</h2></div>
                ...
                <div className="overflow-x-auto w-full flex-1">
                  <table className="w-full text-left min-w-[800px]">
                    <thead><tr className="text-[10px] font-black text-slate-400 uppercase tracking-widest border-b border-slate-200"><th className="pb-4 px-4">產品序號 (S/N)</th><th className="pb-4 px-4">部署單位 / 棟別</th><th className="pb-4 px-4">核心 MAC 參數</th><th className="pb-4 px-4">狀態標籤</th><th className="pb-4 px-4 text-right">操作</th></tr></thead>
                    <tbody className="divide-y divide-slate-100 text-sm text-slate-700">
                      {pendingRecords.map((record, idx) => (
                        <tr key={idx} className="hover:bg-slate-50/50 transition-all">
                          <td className="p-4 font-mono text-xs text-slate-400">{record.產品序號}</td>
                          <td className="p-4 font-bold"><p className="text-slate-800">{record.使用單位}</p><p className="text-[10px] text-slate-400 font-normal uppercase">{record.棟別} | {record.樓層} | {record.裝機日期}</p></td>
                          <td className="p-4"><p className="font-bold text-slate-600">{record.設備類型}</p><p className="text-[10px] font-mono text-blue-600 uppercase font-black">{record.主要mac}</p></td>
                          <td className="p-4"><span className="bg-amber-100 text-amber-700 text-[10px] px-3 py-1.5 rounded-full border border-amber-200 font-black uppercase tracking-widest shadow-sm">待核定</span></td>
                          <td className="p-4 text-right"><button onClick={() => handleDeletePending(record.產品序號)} className="text-[10px] font-black uppercase tracking-widest px-4 py-2 border border-slate-200 text-red-500 rounded-xl hover:bg-red-50 hover:border-red-200 transition-all shadow-sm bg-white">撤回申請</button></td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
