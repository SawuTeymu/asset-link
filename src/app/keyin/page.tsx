 ...
 * 狀態：V5.1 旗艦完全體 (修復 unused-vars 報警)
 * 物理職責：
 * 1. 實裝「載入修正」：提取退回案件還原至表單，並物理覆蓋舊紀錄。
 * 2. 實裝「確認結案」：廠商確認 IP/名稱無誤後，物理遷移至歷史大數據。
 * 3. 修復 try-catch 中未使用的 err 變數，符合 ESLint 嚴格規範。
 * ==========================================
 */
...
function KeyinContent() {
  ...
  const renderCalendarDays = () => {
    ...
    for (let j = 1; j <= daysInMonth; j++) {
      ...
      elements.push(<div key={`day-${j}`} onClick={() => setSelectedDate(dateStr)} className={`h-10 flex items-center justify-center text-sm rounded-xl cursor-pointer font-bold ${dateStr === selectedDate ? 'bg-[#2563eb] text-white shadow-md shadow-blue-500/30' : 'text-slate-500 hover:bg-blue-50'}`}>{j}</div>);
    }
    return elements;
  };
  ...
  const handleLoadRejected = (item: ProgressRecord) => {
    setConfirmDialog({
      ...
      onConfirm: () => {
        ...
        showToast("已載入表單，請修正錯誤後點擊確認提交", "success");
      }
    });
  };
  ...
  const handleConfirmAsset = async (sn: string) => {
    setConfirmDialog({
      ...
      message: "確認資訊室配發之 IP 與名稱正確無誤，並正式結案？",
      type: "info",
      onConfirm: async () => {
  ...
  const clearAll = () => {
    setConfirmDialog({
      ...
      onConfirm: () => {
        // 🚀 修復 unused-vars (Line 203)：加入 console.warn 記錄狀態
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (res.success) {
        // 🚀 修復 unused-vars (Line 234)：加入 console.warn 記錄狀態
        try { localStorage.removeItem(`AL_KEYIN_V0_${vendorName}`); } catch(err: unknown) { console.warn("清理草稿失敗", err); }
        showToast("✅ 預約提交成功，即將重置...", "success");
        setTimeout(() => window.location.reload(), 2000);
      }
    } catch (err: unknown) { 
      // 🚀 修復 unused-vars (Line 238)：加入型別解構與 console.error 日誌
    ...
    <div className="bg-[#f7f9fb] min-h-screen text-[#191c1e] font-[family-name:-apple-system,BlinkMacSystemFont,'SF_Pro_TC','PingFang_TC',system-ui,sans-serif] text-[10.5px] antialiased tracking-tight overflow-x-hidden">
      ...
      <main className="lg:ml-64 pt-24 px-4 sm:px-8 lg:px-10 pb-12">
          ...
          <div className="grid grid-cols-1 md:grid-cols-12 gap-6">
              <div className="col-span-1 md:col-span-4 space-y-6">
                  <div className="glass-card p-6 sm:p-8 rounded-[2rem]">
                      <div className="flex justify-between items-center mb-6">
                          <h3 className="font-black text-slate-800 flex items-center gap-2 text-base"><span className="material-symbols-outlined text-blue-600">calendar_month</span>裝機日期 (C)</h3>
                          <span className="px-3 py-1 bg-blue-100 text-blue-700 text-xs font-bold rounded-full">{new Date().getFullYear()}年 {new Date().getMonth() + 1}月</span>
                      </div>
                      ...
                      <div className="mt-6 pt-6 border-t border-slate-200/50">
                          <div>
                              <span className="text-[10px] font-black text-blue-600 uppercase tracking-widest mb-2 block">VDS 派工單號</span>
                              <div className="compact-input font-mono text-sm text-center bg-slate-50/50">{vdsId}</div>
                          </div>
                      </div>
                  </div>
                  ...
                  <div className="glass-card p-6 sm:p-8 rounded-[2rem]">
                      ...
                      <div className="space-y-5">
                          <div className="grid grid-cols-2 gap-5">
                              <div>
                                  <label className="saas-label" htmlFor="d-area">棟別 (D)</label>
                                  <select id="d-area" value={area} onChange={(e) => setArea(e.target.value)} className="compact-input cursor-pointer">{["A","B","C","D","E","G","H","I","K","T"].map((v) => <option key={v} value={v}>{v} 棟</option>)}<option value="OTHER">其他</option></select>
                                  {area === "OTHER" && <input id="d-area-other" value={areaOther} onChange={(e) => setAreaOther(e.target.value)} placeholder="手動輸入" className="compact-input mt-2" />}
                              </div>
                              <div>
                                  <label className="saas-label" htmlFor="d-floor">樓層 (E)</label>
                                  <input id="d-floor" value={floor} onBlur={(e) => setFloor(formatFloor(e.target.value))} onChange={(e) => setFloor(e.target.value)} className="compact-input" placeholder="例: 05" />
                              </div>
                          </div>
                          <div><label className="saas-label" htmlFor="d-unit">單位 (F)</label><input id="d-unit" value={unit} onChange={(e) => setUnit(e.target.value)} className="compact-input" placeholder="單位全銜" /></div>
                          <div><label className="saas-label !text-blue-600" htmlFor="d-app">姓名 (G)</label><input id="d-app" value={applicant} onChange={(e) => setApplicant(e.target.value)} className="compact-input bg-blue-50/30" placeholder="申請人" /></div>
                      </div>
                  </div>
              </div>
              ...
              <div className="col-span-1 md:col-span-8">
                  <div className="space-y-5">
                      {rows.map((r, i) => (
                        <div key={r.id} className="glass-card p-6 md:p-7 border-l-[8px] border-l-primary relative animate-in slide-in-from-left-4 duration-300 rounded-[1.8rem]">
                            {rows.length > 1 && <button onClick={() => setRows(rows.filter(row => row.id !== r.id))} className="absolute -top-3 -right-3 w-8 h-8 bg-white text-slate-400 hover:text-white hover:bg-red-500 rounded-full flex items-center justify-center shadow-md border border-slate-100 transition-all"><span className="material-symbols-outlined text-sm">close</span></button>}
                            ...
                            {r.originalSn && (
                                <div className="mb-4 inline-flex items-center gap-2 bg-amber-50 text-amber-700 px-3 py-1.5 rounded-lg border border-amber-200">
                                    ...
                                    <span className="text-[10px] font-black uppercase tracking-widest">正在修正退回單: {r.originalSn}</span>
                                </div>
                            )}
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-12 gap-5 md:gap-6">
                                <div className="col-span-1 md:col-span-4">
                                    ...
                                    <div className="type-toggle"><button onClick={() => { const n = [...rows]; n[i].type = "NEW"; setRows(n); }} className={`type-btn ${r.type === 'NEW' ? 'active' : ''}`}>🆕 新購</button><button onClick={() => { const n = [...rows]; n[i].type = "REPLACE"; setRows(n); }} className={`type-btn ${r.type === 'REPLACE' ? 'active !text-orange-600' : ''}`}>🔄 汰換</button></div>
                                </div>
                                <div className="col-span-1 md:col-span-2"><label className="saas-label" htmlFor={`ext-${r.id}`}>分機</label><input id={`ext-${r.id}`} value={r.ext} onChange={(e) => { const n = [...rows]; n[i].ext = e.target.value; setRows(n); }} className="compact-input font-mono" /></div>
                                <div className="col-span-1 md:col-span-6"><label className="saas-label" htmlFor={`model-${r.id}`}>型號 (I)</label><input id={`model-${r.id}`} value={r.model} onChange={(e) => { const n = [...rows]; n[i].model = e.target.value; setRows(n); }} className="compact-input" /></div>
                                <div className="col-span-1 md:col-span-12"><label className="saas-label !text-red-500" htmlFor={`sn-${r.id}`}>序號 S/N (J)</label><input id={`sn-${r.id}`} value={r.sn} onChange={(e) => { const n = [...rows]; n[i].sn = e.target.value.toUpperCase(); setRows(n); }} className="compact-input font-mono uppercase bg-white/90 text-red-600" maxLength={12} /></div>
                                
                                {r.type === "REPLACE" && (
                                  <div className="col-span-1 md:col-span-12"><div className="p-4 bg-amber-50/40 rounded-2xl border border-amber-200/50"><label className="saas-label !text-amber-700 italic" htmlFor={`old-${r.id}`}>原舊機指紋</label><input id={`old-${r.id}`} value={r.oldInfo} onChange={(e) => { const n = [...rows]; n[i].oldInfo = e.target.value; setRows(n); }} className="compact-input" placeholder="IP、MAC 或標籤" /></div></div>
                                )}

                                <div className="col-span-1 md:col-span-6"><label className="saas-label !text-blue-500" htmlFor={`mac1-${r.id}`}>主要 MAC</label><input id={`mac1-${r.id}`} value={r.mac1} onChange={(e) => { const n = [...rows]; n[i].mac1 = e.target.value.toUpperCase().replace(/[^0-9A-F:-]/g, ''); setRows(n); }} onBlur={(e) => { const n = [...rows]; n[i].mac1 = formatMAC(e.target.value); setRows(n); }} className="compact-input font-mono uppercase" maxLength={17} /></div>
                                <div className="col-span-1 md:col-span-6"><label className="saas-label !text-emerald-500" htmlFor={`mac2-${r.id}`}>無線 MAC</label><input id={`mac2-${r.id}`} value={r.mac2} onChange={(e) => { const n = [...rows]; n[i].mac2 = e.target.value.toUpperCase().replace(/[^0-9A-F:-]/g, ''); setRows(n); }} onBlur={(e) => { const n = [...rows]; n[i].mac2 = formatMAC(e.target.value); setRows(n); }} className="compact-input font-mono uppercase" maxLength={17} /></div>
                            </div>
                        </div>
                      ))}
                  </div>
                  <button onClick={() => setRows([...rows, { id: Date.now(), model: "", sn: "", mac1: "", mac2: "", ext: "", oldInfo: "", type: "NEW" }])} className="mt-5 w-full py-8 border-2 border-dashed border-blue-300 rounded-[2rem] flex flex-col items-center justify-center text-blue-600 bg-blue-50/40 hover:bg-blue-100 transition-all font-black text-sm"><span className="material-symbols-outlined text-4xl mb-2">add_circle</span>增加設備項目</button>
              </div>
          </div>
      </main>
            ...
            <div className="glass-card w-full max-w-4xl rounded-[2.5rem] p-6 shadow-2xl flex flex-col max-h-[90vh] bg-white/95">
                <div className="flex justify-between items-center mb-6 pb-4 border-b border-slate-200">
                    <div className="flex items-center gap-4">
                        ...
                        <div><h2 className="text-xl font-black text-slate-800">申請進度查詢與重送</h2><p className="text-[10px] font-bold text-blue-600 uppercase">{vendorName} 的預約軌跡</p></div>
                    </div>
                    <button onClick={() => setIsProgressOpen(false)} className="w-10 h-10 rounded-full bg-slate-100 hover:bg-red-50 text-slate-500 hover:text-red-500 flex items-center justify-center"><span className="material-symbols-outlined text-xl">close</span></button>
                </div>
                ...
                <div className="flex-1 overflow-y-auto pr-2 space-y-4">
                    {isProgressLoading ? (
                        ...
                        progressData.map((item, idx) => (
                            <div key={idx} className="bg-slate-50/50 p-5 rounded-2xl flex flex-col transition-all hover:shadow-md border border-slate-200 mb-3">
                                <div className="flex items-center justify-between mb-3">
                                    <div className="flex items-center gap-3">
                                        <span className="text-sm font-black text-slate-800 font-mono">{item.sn}</span>
                                        <span className={`px-3 py-1 rounded-full text-[10px] font-black border ${item.status === '待核定' ? 'bg-slate-200 text-slate-600 border-slate-300' : item.status === '退回修正' ? 'bg-red-50 text-red-600 border-red-200' : item.status.includes('確認') ? 'bg-blue-50 text-blue-600 border-blue-200 animate-pulse' : 'bg-emerald-50 text-emerald-700 border-emerald-200'}`}>{item.status}</span>
                                    </div>
                                ...
                                {item.status === '退回修正' && (
                                    <div className="mt-2 bg-red-50/60 p-4 rounded-xl border border-red-100">
                                        ...
                                        <button onClick={() => handleLoadRejected(item)} className="w-full py-2.5 bg-red-600 text-white rounded-lg font-black text-xs uppercase hover:bg-red-700 active:scale-95 transition-all shadow-md">載入資料並進行修正</button>
                                    </div>
                                )}

                                {item.status === '已核定(待確認)' && (
                                    <div className="mt-2 bg-blue-50/60 p-4 rounded-xl border border-blue-100">
                                        <p className="text-xs font-black text-blue-800 mb-2">資訊室已核發，請確認配發結果：</p>
                                        <div className="flex flex-col sm:flex-row gap-3 text-[11px] font-bold text-blue-700 mb-4">
                                            <span className="bg-white px-3 py-1.5 rounded-lg border border-blue-100 shadow-sm flex items-center gap-2"><span className="material-symbols-outlined text-[14px]">lan</span>配發 IP：<span className="font-mono font-black">{item.assignedIp}</span></span>
                                            <span className="bg-white px-3 py-1.5 rounded-lg border border-blue-100 shadow-sm flex items-center gap-2"><span className="material-symbols-outlined text-[14px]">dns</span>設備名稱：<span className="font-mono font-black">{item.assignedName}</span></span>
                                        </div>
                                        <button onClick={() => handleConfirmAsset(item.sn)} className="w-full py-2.5 bg-blue-600 text-white rounded-lg font-black text-xs uppercase hover:bg-blue-700 active:scale-95 transition-all shadow-md flex items-center justify-center gap-2">
                                           <span className="material-symbols-outlined text-[16px]">task_alt</span>確認無誤，執行結案入庫
                                        </button>
                                    </div>
                                )}
                            </div>
        ...
        <div className="fixed inset-0 z-[2000] bg-slate-900/40 backdrop-blur-sm flex items-center justify-center p-4 fade-enter">
           <div className={`glass-card w-full max-w-sm rounded-[2rem] overflow-hidden shadow-2xl bg-white/95 border-t-[8px] ${confirmDialog.type === 'danger' ? 'border-t-error' : 'border-t-blue-500'}`}>
              <div className="p-8 text-center space-y-6">
                 <div className={`w-16 h-16 rounded-2xl flex items-center justify-center mx-auto border ${confirmDialog.type === 'danger' ? 'bg-red-50 text-error border-red-100' : 'bg-blue-50 text-blue-600 border-blue-100'}`}>
                 ...
                 <div className="flex gap-3 pt-2">
                    ...
                    <button onClick={() => { confirmDialog.onConfirm(); setConfirmDialog(prev => ({...prev, isOpen: false})); }} className={`flex-1 py-3.5 text-white rounded-xl font-black uppercase ${confirmDialog.type === 'danger' ? 'bg-error' : 'bg-blue-600'}`}>確認執行</button>
                 </div>
              </div>
           </div>
        </div>
      ...
      {isLoading && (
        <div className="fixed inset-0 bg-white/80 z-[600] flex flex-col items-center justify-center backdrop-blur-md">
            <div className="w-12 h-12 border-4 border-t-primary rounded-full animate-spin mb-4 shadow-lg"></div>
            <p className="text-primary font-black tracking-widest uppercase">{loaderText}</p>
        </div>
      )}
      ...
      <div className="fixed bottom-10 left-1/2 -translate-x-1/2 z-[700] flex flex-col gap-2 pointer-events-none w-full max-w-[340px] px-4">
        {toasts.map(t => (
            <div key={t.id} className={`px-6 py-4 rounded-2xl text-xs font-black text-white shadow-2xl animate-bounce flex items-center gap-3 ${t.type === 'error' ? 'bg-red-600' : 'bg-slate-900'}`}><span className="material-symbols-outlined text-base">info</span><span>{t.msg}</span></div>
        ))}
      </div>
    </div>
  );
...
export default function App() {
  return (
    <Suspense fallback={<div className="min-h-screen flex items-center justify-center bg-[#f8fafc]"><div className="w-12 h-12 border-4 border-t-primary rounded-full animate-spin"></div></div>}>
      <KeyinContent />
    </Suspense>
  );
}