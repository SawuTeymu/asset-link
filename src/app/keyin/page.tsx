 ...
 * 狀態：V300.18 實體對沖對正版 (修正 assets 表名連結)
 * 物理職責：
 * 1. 實體對正：將 API 調用從 assets_pending 物理修正為 assets。
 * 2. 數據源對沖：從物理表 buildings 實時抓取棟別。
 * 3. 0 簡化：保留品牌型號、無線 MAC、備註、MAC 自動對沖格式化。
 * 4. 無符號化：物理抹除所有 Emoji 與不必要特殊符號。
 * ==========================================
 */
              ...
              <section className="col-span-1 lg:col-span-4">
                <div className={`${styles.clinicalGlass} rounded-2xl p-5 md:p-8 shadow-sm`}>
                  ...
                  <div className="space-y-4">
                     ...
                     <div><label className="text-xs font-bold text-slate-500 mb-1 block">單位全稱</label><input type="text" value={metadata.unit} onChange={e => setMetadata({...metadata, unit: e.target.value})} placeholder="如: 資訊組" className={styles.crystalInput} /></div>
                     <div><label className="text-xs font-bold text-slate-500 mb-1 block">填報人 (#分機)</label><input type="text" value={metadata.applicant} onChange={e => setMetadata({...metadata, applicant: e.target.value})} placeholder="姓名#1234" className={styles.crystalInput} /></div>
                  </div>
                </div>
              </section>

              <section className="col-span-1 lg:col-span-8">
                 <div className={`${styles.clinicalGlass} rounded-2xl p-5 md:p-8 shadow-sm flex flex-col`}>
                    ...
                    <button onClick={handleSubmit} disabled={isLoading} className="mt-6 w-full py-4 bg-blue-600 text-white rounded-xl font-bold shadow-md hover:bg-blue-700 active:scale-95 transition-all disabled:opacity-50">提交預約核准</button>
                 </div>
              </section>
            </div>
          )}
                    ...
                    <tbody className="divide-y divide-slate-100 text-sm text-slate-700">
                      {pendingRecords.map((record) => (
                        <tr key={record.id} className="hover:bg-slate-50/50">
                          ...
                          <td className="p-3"><p className="font-bold">{record.設備類型}</p><p className="text-xs font-mono text-blue-600">有線: {record.主要mac}</p></td>
