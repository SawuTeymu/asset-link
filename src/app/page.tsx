 ...
 * ==========================================
 * 檔案：src/app/page.tsx
 * 狀態：V67.0 旗艦視覺配色還原體 (100% 對照使用者指定樣式)
 * 物理職責：
 * 1. 視覺還原：植入 primary-fixed-dim 裝飾球與 3XL 極限毛玻璃。
 * 2. 交互對沖：保留 rotate-3 圖標、按鈕 active 縮放與 bounce 報錯。
 * 3. 數據守衛：維持 V2.6 嚴格語法邏輯，0 簡化、0 刪除。
 * ==========================================
 */
...
export default function LoginPage() {
  ...
  useEffect(() => {
    const fetchVendors = async () => {
      try {
        ...
        // 🚀 雙重轉型：物理抹除 Supabase 接口與前端介面的 Parser 衝突
        const typedData = data as unknown as VendorDbRow[] | null;
  ...
  const handleAdminLogin = () => {
    ...
    // 物理驗證：比對 constants.ts 中的帳密陣列 (uid 欄位)
    const isValidAdmin = ADMIN_CREDENTIALS_LIST.some(
      (admin) => admin.uid === adminAccount && admin.password === adminPassword
    );
  ...
  return (
    <div className="min-h-screen bg-[#f7f9fb] flex items-center justify-center p-6 font-sans text-[#191c1e] antialiased overflow-hidden relative">
      ...
      <div className="relative z-10 w-full max-w-[420px]">
        ...
        {/* 登入表單區塊 (還原 bg-white/70 backdrop-blur-3xl) */}
        <div className="bg-white/70 backdrop-blur-3xl rounded-[2.5rem] p-8 shadow-2xl border border-white/50 animate-in fade-in zoom-in-95 duration-700">
          ...
          {!loginType && (
            <div className="space-y-4">
              ...
              >
                ...
                <div>
                  <h3 className="font-black text-slate-800 text-lg">廠商預約申請</h3>
                  <p className="text-[10px] font-bold text-slate-400 uppercase">Vendor Application Portal</p>
                </div>
              </button>
          ...
          {loginType === "admin" && (
            <div className="space-y-5 animate-in fade-in duration-500">
              ...
              <div className="space-y-4">
                <div className="space-y-2">
                  <label htmlFor="adminAccountInput" className="text-[10px] font-black text-slate-400 uppercase tracking-widest pl-1">管理帳號</label>
                  <input 
                ...
                <div className="space-y-2">
                  <label htmlFor="adminPasswordInput" className="text-[10px] font-black text-slate-400 uppercase tracking-widest pl-1">管理密碼</label>
                  <input 
          ...
          {loginType === "vendor" && (
            <div className="space-y-5 animate-in fade-in duration-500">
              ...
              <div className="space-y-2">
                <label htmlFor="vendorSelectInput" className="text-[10px] font-black text-slate-400 uppercase tracking-widest pl-1">廠商名稱 (Vendor ID)</label>
                <div className="relative">
                  <select 
                    ...
                    onChange={(e) => setSelectedVendor(e.target.value)}
                    title="選擇廠商"
                    className="w-full appearance-none bg-slate-50 border-2 border-slate-100 rounded-2xl px-5 py-4 font-bold text-slate-700 outline-none focus:border-primary focus:bg-white transition-all shadow-sm cursor-pointer"
                  >
                    <option value="" disabled>請選擇您的廠商名稱...</option>
                    {vendors.map(v => (
                      <option key={v.name} value={v.name}>
                        {v.name} {v.status === '停用' || v.status === '停權' ? '(🚫 已凍結)' : ''}
                      </option>
                    ))}
                  </select>
                  <span className="material-symbols-outlined absolute right-5 top-1/2 -translate-y-1/2 text-slate-400 pointer-events-none">expand_more</span>
              ...
              <button 
                ...
                className="w-full bg-primary text-white rounded-2xl py-4 font-black uppercase tracking-widest hover:bg-primary/90 active:scale-95 transition-all shadow-xl shadow-primary/20 disabled:opacity-50 flex items-center justify-center gap-2 mt-2"
              >
                {isLoading ? <span className="material-symbols-outlined animate-spin text-sm">refresh</span> : '進入作業區域'}
              </button>
            </div>
          )}
