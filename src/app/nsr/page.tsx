...
import { submitNsrRequest, getNsrRecords, markNsrBilled, batchMarkNsrBilled } from "@/lib/actions/nsr";
...
export default function NsrPage() {
  ...
  const fetchRecords = useCallback(async () => {
    setIsLoading(true);
    try {
      const data = await getNsrRecords(statusFilter);
      setRecords(data || []);
    } catch (err: any) {
  ...
  useEffect(() => {
    ...
    fetchBuildings();
    if (activeTab === "settlement") fetchRecords();
  }, [router, activeTab, fetchBuildings, fetchRecords]);
  ...
  const handleSubmitNsr = async () => {
    ...
    try {
      ...
      const res = await submitNsrRequest(payload);
      if (res.success) {
        setRequests([{ type: "新設網點", location: "", sn: "", remark: "" }]);
        showToast("NSR 網點申請已成功錄入，請至結算區查看", "success");
      }
    } catch (err: any) {
  ...
  const handleMarkBilled = async (sn: string) => {
    if (!confirm(`確定要將單號 ${sn} 標記為『已計價結算』嗎？`)) return;
    setIsLoading(true);
    try {
  ...
  const handleBatchMarkBilled = async () => {
    ...
    setIsLoading(true);
    try {
      const sns = unbilledRecords.map(r => r.sn);
                    ...
                    <div className="flex flex-col gap-5 w-full">
                      {requests.map((r, i) => (
                        <div key={i} className={styles.deviceItemBlock}>
                          ...
                          <div className={styles.rowGrid}>
                            <div><label className={styles.inputLabel}>申請單號 (留空自動產生)</label><input placeholder="C012026..." value={r.sn} onChange={e => { const nr = [...requests]; nr[i].sn = e.target.value.toUpperCase(); setRequests(nr); }} className={`${styles.crystalInput} font-mono text-red-600`} /></div>
          ...
          <div className="animate-in fade-in zoom-in-95 duration-300">
            <header className="mb-10 flex flex-col md:flex-row md:items-center justify-between gap-4 mt-12 md:mt-0">
              ...
              <div className="flex gap-2">
                 ...
                 <button onClick={fetchRecords} className="text-[10px] font-black uppercase tracking-widest px-4 py-3 bg-white text-slate-600 border border-slate-200 rounded-xl shadow-sm hover:text-blue-600 hover:border-blue-200 transition-all flex items-center gap-2">
            ...
            <div className={`${styles.clinicalGlass} rounded-3xl p-6 md:p-8 shadow-sm flex flex-col min-h-[500px]`}>
              <div className="flex items-center justify-between mb-8 border-b border-slate-100 pb-4">
                <div className="flex items-center gap-2">
                  ...
                  <h2 className="text-lg font-bold text-slate-800 tracking-tight">獨立 NSR 計價庫紀錄</h2>
                </div>
              ...
              <div className={styles.tableContainer}>
                <table className={`w-full text-left lg:min-w-[1000px] ${styles.responsiveTable}`}>
                  ...
                  <tbody className="divide-y divide-slate-100 text-sm text-slate-700">
                    ...
                    {records.length === 0 && !isLoading && <tr><td colSpan={5} className="py-20 text-center text-slate-400 font-bold italic border-none">找不到符合條件的 NSR 計價紀錄。</td></tr>}
                  </tbody>
                </table>
              </div>
            </div>
